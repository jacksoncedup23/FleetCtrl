...
use function PHPSTORM_META\map;
	...
	$SQLContPages = 'SELECT CEIL(COUNT(*) /' . $linesPerPage . ') as CONT, COUNT(*) as TOTAL FROM tbUsuarios u WHERE 1=1 ';
	if (isset($_GET['search']))
	{
		...
		$SQLContPages = $SQLContPages . " and (u.NOME LIKE '%$busca%' or u.USUARIO LIKE '%$busca%' or u.EMAIL LIKE '%$busca%' or u.ID='$busca')";
		}
	}
...
<html lang="pt-br">
	...
	<!-- --------------------------- -->
									...
									<tbody>
										<?php while($users = mysqli_fetch_assoc($result_users)){
											...
											<tr>
												...
												<td class="align-middle"><span class="lnkConsulta" data-bs-toggle="modal" data-bs-target="#ConsultaUser" onclick="consulta_user(<?php echo $users['ID']; ?>)"><?php echo $users['NOME']; ?></span></td>
												...
												<td class="align-middle">
													<button type="button" class="btn btn-xs btn-primary" data-bs-toggle="modal" data-bs-target="#ConsultaUser" onclick="consulta_user(<?php echo $users['ID']; ?>)"><img src="./images/visualizar.png"></button>
		...
		<div class="modal fade" id="ConsultaUser" tabindex="-1" aria-labelledby="TitleConsulta" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content" id="divContentConsulta">
	...
	<script>
		...
		function consulta_user(id_user) {
			$.post("query_consulta_usuario.php", {id:id_user}, function(x) { $("#divContentConsulta").html(x); } );
		}
